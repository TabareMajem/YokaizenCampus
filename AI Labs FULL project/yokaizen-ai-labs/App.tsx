...
export const App: React.FC = () => {
  ...
  const handleCreateSquad = (name: string, avatar: string) => {
      if (!user) return;
      const newSquad: Squad = { 
          ...
          avatar, 
          tier: 'Rookie', 
          members: [{ id: user.id, name: user.name, avatar: user.avatar, role: 'leader' }], 
          totalXp: 0, 
          weeklyQuestTarget: 1000, 
  ...
  const handleJoinSquad = (id: string) => {
      ...
      const updatedSquads = squads.map(s => {
          if (s.id === id) {
              if (s.members.some(m => m.id === user.id)) return s;
              return {
                  ...s,
                  members: [...s.members, { id: user.id, name: user.name, avatar: user.avatar, role: 'member' as const }]
              };
          }
          if (s.id === user.squadId) {
               return {
                   ...s,
                   members: s.members.filter(m => m.id !== user.id)
               };
          }
          return s;
      });
  ...
  return (
      <div className="flex h-screen bg-[#050505] text-white overflow-hidden font-sans selection:bg-electric/30">
          ...
          {/* Sidebar */}
          <nav className="w-64 bg-black/90 border-r border-white/10 flex-col hidden md:flex z-50">
              ...
              <div className="flex-1 space-y-1 p-4">
                  {[AppTab.HOME, AppTab.LEARN, AppTab.LAB, AppTab.LEADERBOARD, AppTab.PROFILE].map(tab => (
                      ...
                      >
                          {tab === AppTab.HOME && <Layout size={20} className="mr-3"/>}
                          {tab === AppTab.LEARN && <BookOpen size={20} className="mr-3"/>}
                          {tab === AppTab.LAB && <FlaskConical size={20} className="mr-3"/>}
                          {tab === AppTab.LEADERBOARD && <Trophy size={20} className="mr-3"/>}
                          {tab === AppTab.PROFILE && <User size={20} className="mr-3"/>}
          ...
          <main className="flex-1 flex flex-col relative overflow-hidden">
                  ...
                  {activeTab === AppTab.HOME && (
                      <div className="p-6 pb-24 max-w-7xl mx-auto animate-in fade-in">
                          ...
                          <div className="mb-8 relative rounded-2xl overflow-hidden border border-white/10 bg-gray-900 shadow-2xl group">
                              ...
                              <div className="absolute inset-0 bg-gradient-to-r from-purple-900/40 via-black/50 to-blue-900/40"></div>
                                      ...
                                      <div className="absolute bottom-3 left-3 right-3 z-10">
                                          ...
                                          <div className="flex items-center space-x-2">
                                              <span className={`text-[9px] px-2 py-0.5 rounded font-bold uppercase border ${
                                                  game.difficulty === 'Rookie' ? 'bg-green-900/40 border-green-500/30 text-green-400' : 
                      ...
                      <LearnPath 
                          ...
                          onNodeClick={(node) => {
                              if (node.gameId) {
                                  ...
                                  alert("Lesson Module: " + node.title);
                              }
                          }} 
                      />
                  )}
              ...
              <div className="md:hidden bg-black border-t border-white/10 flex justify-around p-3 z-50 pb-safe">
                  {[AppTab.HOME, AppTab.LEARN, AppTab.LAB, AppTab.LEADERBOARD, AppTab.PROFILE].map(tab => (
                      ...
                      >
                          {tab === AppTab.HOME && <Layout size={20}/>}
