...
export class AudioService {
  ...
  public playSystemMessage(options?: { type: 'warning' | 'success' | 'info' | 'error' }) {
    if (!this.ctx || !this.masterGain) return;
    try {
      if (options?.type === 'warning' || options?.type === 'error') {
        this.createOscillator('square', 300, 0.1, 0.05);
        setTimeout(() => this.createOscillator('square', 250, 0.1, 0.05), 100);
        this.vibrate(this.haptics.failure);
      } else {
        this.createOscillator('triangle', 600, 0.05, 0.05);
        setTimeout(() => this.createOscillator('triangle', 800, 0.05, 0.05), 100);
        this.vibrate(this.haptics.success);
      }
    } catch (e) { }
  }
