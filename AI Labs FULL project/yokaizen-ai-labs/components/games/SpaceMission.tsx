...
const TARGET_COLORS = ['#4466ff', '#ff6644', '#6644ff', '#ffffff'];
...
export const SpaceMission: React.FC<SpaceMissionProps> = ({ onComplete, difficulty, t }) => {
    ...
    const handleTargetClick = useCallback((id: string) => {
        ...
        setScreenFlash('#4466ff'); setTimeout(() => setScreenFlash(null), 80);
        ...
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden border border-white/10 bg-black shadow-2xl">
            ...
            <div className="absolute inset-0 z-0 cursor-crosshair">
                <Canvas camera={{ position: [0, 0, 12], fov: 60 }} gl={{ antialias: false, powerPreference: "high-performance" }}>
                    <color attach="background" args={['#000204']} />
                    <fog attach="fog" args={['#000204', 8, 25]} />
                    <ambientLight intensity={0.3} />
                    <pointLight position={[10, 10, 10]} intensity={2} color="#4466ff" />
                    <pointLight position={[-10, -10, -10]} intensity={1} color="#ff6644" />
                    ...
                    <Sparkles count={300} scale={20} size={3} speed={0.5 + combo * 0.2} opacity={0.5} color="#4466ff" />
                    <Sparkles count={100} scale={15} size={6} speed={1} opacity={0.3} color="#ff6644" />
