                 ...
                 <div className="relative w-[300px] h-[300px] transform rotate-x-60 rotate-z-45" style={{ transform: 'rotateX(60deg) rotateZ(-45deg)' }}>
                      {WORLD_MAP.map((row, y) => row.map((cell, x) => {
                          ...
                          return (
                              ...
                              >
                                  {isPlayer && <div className="absolute -top-8 left-1/2 -translate-x-1/2 z-20"><div className="w-4 h-8 bg-cyan shadow-[0_0_10px_#00FFFF]"></div></div>}
            ...
            <div className="absolute inset-0 bg-gray-950 z-30 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 relative flex flex-col items-center justify-center">
                    ...
                    <div className="mb-8 text-center relative w-full max-w-xs mx-auto">
                        <div className={`w-32 h-32 mx-auto bg-red-900/20 border-4 ${isEnemyStunned ? 'border-yellow-400' : 'border-red-500/50'} rounded-full flex items-center justify-center mb-2 relative`}>
                            {timelineStability < 40 && <div className="absolute inset-0 bg-red-500/20 animate-ping rounded-full"></div>}
                            <Zap size={64} className={`${isEnemyStunned ? 'text-yellow-400 rotate-180' : 'text-red-500'} transition-all`} />
                        </div>
                        <div className="text-red-500 font-bold text-xl tracking-widest">{t('chrono.enemy')}</div>
                        <div className="w-48 h-3 bg-gray-800 rounded-full mx-auto mt-2 overflow-hidden border border-gray-700">
                            <div className="h-full bg-red-600 transition-all duration-500" style={{ width: `${(enemyHP / enemyMaxHP) * 100}%` }}></div>
                        </div>
                        <div className="text-xs text-red-400 mt-1 font-mono">{enemyHP}/{enemyMaxHP} {t('chrono.hp')}</div>
                    </div>

                    {/* Player Card */}
                    <div className="flex items-center space-x-6 bg-black/50 p-4 rounded-2xl border border-white/10 backdrop-blur shadow-2xl">
                        <div className="text-center">
                            <div className="text-[10px] text-gray-400 uppercase mb-1">{t('chrono.hp')}</div>
                            <div className="text-2xl font-bold text-green-400 flex items-center justify-center font-mono">
                                <Heart size={18} className="mr-2 fill-green-400"/> {player.hp}
                            </div>
                        </div>
                    ...
                    {MOVES.map(m => {
                        ...
                        return (
                            <button
                                ...
                                className={`p-3 rounded-xl border text-left transition-all active:scale-95 flex flex-col relative overflow-hidden ${
                                    playerTurn && canAfford
