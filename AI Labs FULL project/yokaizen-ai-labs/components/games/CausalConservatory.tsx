...
export interface CausalConservatoryProps {
...
const HazardNode = ({ position, onClick, id }: {
    ...
    return (
        <mesh ref={meshRef} position={position}
            ...
            <pointLight distance={4} intensity={2.5} color="#ff4400" />
        </mesh>
    );
};
...
const TARGET_COLORS = ['#88ff00', '#00ff88', '#00aa44', '#ffffff'];
const genPos = (): [number, number, number] => [(Math.random() - 0.5) * 12, (Math.random() - 0.5) * 8, (Math.random() - 0.5) * 6];

// --- Main Component ---
export const CausalConservatory: React.FC<CausalConservatoryProps> = ({ onComplete, difficulty, t }) => {
    ...
    const [adversaryMsg, setAdversaryMsg] = useState(t('game.adversary.core_exposed'));
    ...
    useEffect(() => {
        queueDialogue([
            { id: `causalconservatory-brief-${Date.now()}`, character: 'SYNTAX', text: t('game.instructions.click_targets') },
            { id: `causalconservatory-brief2-${Date.now()}`, character: 'BYTE', text: t('game.instructions.avoid_hazards'), isGlitchy: true },
        ]);
    }, [queueDialogue, t]);
    ...
    useEffect(() => {
        if (gameState !== 'PLAYING') return;
        const aLines = [t('game.advisor.wave_incoming'), t('game.advisor.core_holding'), t('game.advisor.diversion_nodes'), `${t('game.hud.combo_chain')}: ${combo}x`];
        const eLines = [t('game.adversary.core_exposed'), t('game.adversary.wont_survive'), t('game.adversary.same_look'), `${combo}x ${t('game.hud.combo')}...`];
                ...
                if (prev <= 1) {
                    ...
                    if (fs === 'SUCCESS') {
                        audio.playSuccess(); queueDialogue([
                            { id: `causalconservatory-win-${Date.now()}`, character: 'SYNTAX', text: `${t('game.hud.final_score')}: ${score}. ${t('game.hud.max_combo')}: ${maxCombo}x.` },
                            { id: `causalconservatory-win2-${Date.now()}`, character: 'ATHENA', text: t('game.state.waves_analyzed') },
                        ]);
                    } else {
                        audio.playError(); queueDialogue([
                            { id: `causalconservatory-fail-${Date.now()}`, character: 'BYTE', text: t('game.state.core_breached'), isGlitchy: true },
                        ]);
                    }
                    onComplete(score, { completionTime: 55 - prev, difficulty, maxCombo });
                    return 0;
                }
                return prev - 1;
            });
    ...
    const handleTargetClick = useCallback((id: string) => {
        ...
        setScreenFlash('#88ff00'); setTimeout(() => setScreenFlash(null), 80);
        ...
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden border border-white/10 bg-black shadow-2xl">
            ...
            <div className="absolute inset-0 z-0 cursor-crosshair">
                <Canvas camera={{ position: [0, 0, 12], fov: 60 }} gl={{ antialias: false, powerPreference: "high-performance" }}>
                    <color attach="background" args={['#020800']} />
                    <fog attach="fog" args={['#020800', 8, 25]} />
                    <ambientLight intensity={0.3} />
                    <pointLight position={[10, 10, 10]} intensity={2} color="#88ff00" />
                    <pointLight position={[-10, -10, -10]} intensity={1} color="#00aa44" />
                    ...
                    <Sparkles count={300} scale={20} size={3} speed={0.5 + combo * 0.2} opacity={0.5} color="#88ff00" />
                    <Sparkles count={100} scale={15} size={6} speed={1} opacity={0.3} color="#00ff88" />
