...
const ScanTarget: React.FC<{ x: number; y: number; label: string; onClick: () => void }> = ({ x, y, label, onClick }) => (
    <button onClick={onClick} className="absolute w-8 h-8 border-2 border-green-400 rounded-full flex items-center justify-center group hover:scale-110 transition-transform z-20 bg-black/20 backdrop-blur-sm" style={{ left: `${x}%`, top: `${y}%` }}><div className="w-1 h-1 bg-green-400"></div><div className="absolute top-full mt-2 bg-black text-green-400 text-[10px] px-2 py-1 font-mono border border-green-400 opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap">{t('games.aethelredgambit.scan')}{label}</div></button>
);
...
export const AethelredGambit: React.FC<AethelredGambitProps> = ({ onComplete, t }) => {
    ...
    const handleAction = async (action: 'DEBATE' | 'SCAN', payload?: string) => {
        ...
        try {
            ...
            if (result.corruption <= 0) { setTimeout(() => { audio.playSuccess(); showToast("SECTOR RESTORED. LOGIC STABILIZED.", 'success'); setSector('HUB'); }, 4000); }
        ...
        <div className="h-full bg-black flex flex-col relative overflow-hidden font-mono select-none">
            ...
            {sector === 'HUB' && (
                <div className="flex-1 flex flex-col items-center justify-center p-8 z-10 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')]">
                    ...
                    <h1 className="text-4xl font-black text-white mb-2 text-center tracking-tighter uppercase">{t('games.aethelredgambit.the_fractured')}<span className="text-blue-500">{t('games.aethelredgambit.continuum')}</span></h1>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl">
                        <button onClick={() => enterSector('ALPHA')} className="group relative aspect-video border-4 border-amber-600 bg-amber-950 overflow-hidden hover:scale-105 transition-transform"><div className="absolute inset-0 bg-[url('https://picsum.photos/seed/western/400/300')] bg-cover opacity-50 grayscale group-hover:grayscale-0 transition-all"></div><div className="absolute inset-0 flex flex-col items-center justify-center p-4 bg-black/50"><div className="text-amber-500 font-bold text-xl mb-1">{t('games.aethelredgambit.alpha')}</div><div className="text-white text-xs">{SECTORS.ALPHA.name}</div></div></button>
                        <button onClick={() => enterSector('BETA')} className="group relative aspect-video border-4 border-cyan-600 bg-cyan-950 overflow-hidden hover:scale-105 transition-transform"><div className="absolute inset-0 bg-[url('https://picsum.photos/seed/cyberpunk/400/300')] bg-cover opacity-50 grayscale group-hover:grayscale-0 transition-all"></div><div className="absolute inset-0 flex flex-col items-center justify-center p-4 bg-black/50"><div className="text-cyan-500 font-bold text-xl mb-1">{t('games.aethelredgambit.beta')}</div><div className="text-white text-xs">{SECTORS.BETA.name}</div></div></button>
                        <button onClick={() => enterSector('GAMMA')} className="group relative aspect-video border-4 border-red-600 bg-red-950 overflow-hidden hover:scale-105 transition-transform"><div className="absolute inset-0 bg-[url('https://picsum.photos/seed/mars/400/300')] bg-cover opacity-50 grayscale group-hover:grayscale-0 transition-all"></div><div className="absolute inset-0 flex flex-col items-center justify-center p-4 bg-black/50"><div className="text-red-500 font-bold text-xl mb-1">{t('games.aethelredgambit.gamma')}</div><div className="text-white text-xs">{SECTORS.GAMMA.name}</div></div></button>
                    </div>
                </div>
            )}
            {sector !== 'HUB' && (
                <div className={`flex-1 flex flex-col relative bg-gradient-to-b ${SECTORS[sector].bgGradient}`}>
                    <div className="flex justify-between items-center p-3 bg-black/80 border-b border-white/20 z-20 backdrop-blur-md"><div className="flex items-center space-x-3"><Button size="sm" variant="ghost" onClick={() => setSector('HUB')}><ArrowLeft size={16} /></Button><div><div className={`text-xs font-bold text-${SECTORS[sector].theme}-400`}>{SECTORS[sector].name}</div><div className="text-[10px] text-gray-400">{SECTORS[sector].aiName} {t('games.aethelredgambit.corruption')}{corruption}%</div></div></div><div className="w-32 h-3 bg-gray-800 border border-gray-600 rounded overflow-hidden"><div className={`h-full transition-all duration-500 ${corruption > 60 ? 'bg-red-500 animate-pulse' : 'bg-green-500'}`} style={{ width: `${corruption}%` }}></div></div></div>
                    <div className="flex-1 relative overflow-hidden flex items-center justify-center p-4"><div className="absolute inset-0 opacity-30 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] mix-blend-overlay pointer-events-none"></div><div className="relative z-10 flex flex-col items-center animate-float"><Avatar seed={SECTORS[sector].aiName || 'unknown'} glitch={corruption > 50} /></div>
                        {realityDeckActive && (<div className="absolute inset-0 z-20 bg-green-900/20 pointer-events-auto"><div className="absolute inset-0 bg-[linear-gradient(rgba(0,255,0,0.1)_1px,transparent_1px),linear-gradient(90deg,rgba(0,255,0,0.1)_1px,transparent_1px)] bg-[length:40px_40px]"></div>{SECTORS[sector].hotspots?.map(h => (<ScanTarget key={h.id} x={h.x} y={h.y} label={h.label} onClick={() => handleAction('SCAN', h.label)} />))}<div className="absolute top-4 left-1/2 -translate-x-1/2 bg-black text-green-400 px-4 py-1 border border-green-500 text-xs font-bold animate-pulse">{t('games.aethelredgambit.reality_deck_active')}</div></div>)}
                    </div>
