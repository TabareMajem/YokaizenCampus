...
export const GlitchwaveAnalyst: React.FC<GlitchwaveAnalystProps> = ({ onComplete, t }) => {
  ...
  useEffect(() => {
      ...
      const render = () => {
          ...
          // Draw Combined Wave (Visualizing the noise clearing up)
          ctx.beginPath();
          ctx.lineWidth = 2;
          
          for (let x = 0; x < width; x++) {
              // Theoretical perfect wave
              ...
              // We draw the "Result" signal
              const drawY = targetY + noiseY; // If perfect match, noiseY is 0, we see clean target wave.

              // Color based on sync
              const syncColor = diff < 20 ? '#00ff00' : '#005500';
              ctx.strokeStyle = gameState === 'LOCKED' ? '#ffffff' : syncColor;
          ...
          // If totalDiff is low, we draw the hidden image with higher opacity
          const accuracy = Math.max(0, 1 - (totalDiff / (width * 100))); // 0 to 1
          
          if (accuracy > 0.1) {
              // Simulate image reveal
              ctx.globalAlpha = accuracy * (Math.random() * 0.5 + 0.5); // Flicker
