...
const Rain = ({ count, intensity, t = (k: any) => k }: { count: number, intensity: number }) => {
...
const SceneController = ({ state }: { state: CyberRainState }) => {
    return (
        <>
            ...
            <CyberpunkEffects
                bloomIntensity={state.rainIntensity > 0.5 ? 2.5 : 1.5}
                glitchFactor={state.fogDensity > 0.05 ? 0.005 : 0}
            />
        </>
    );
};
