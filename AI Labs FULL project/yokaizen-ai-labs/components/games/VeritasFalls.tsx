...
const TARGET_COLORS = ['#8844ff', '#ff4488', '#44ff88', '#ffffff'];
...
export const VeritasFalls: React.FC<VeritasFallsProps> = ({ onComplete, difficulty, t }) => {
    ...
    useEffect(() => {
        queueDialogue([
            { id: `veritasfalls-brief-${Date.now()}`, character: 'BYTE', text: t('game.instructions.click_targets') },
            { id: `veritasfalls-brief2-${Date.now()}`, character: 'SYNTAX', text: t('game.instructions.avoid_hazards'), isGlitchy: true },
        ]);
    }, [queueDialogue, t]);
                ...
                if (prev <= 1) {
                    ...
                    if (fs === 'SUCCESS') {
                        audio.playSuccess(); queueDialogue([
                            { id: `veritasfalls-win-${Date.now()}`, character: 'SYNTAX', text: `${t('game.hud.final_score')}: ${score}. ${t('game.hud.max_combo')}: ${maxCombo}x.` },
                            { id: `veritasfalls-win2-${Date.now()}`, character: 'ATHENA', text: t('game.state.system_secured') },
                        ]);
                    } else {
                        audio.playError(); queueDialogue([
                            { id: `veritasfalls-fail-${Date.now()}`, character: 'BYTE', text: t('game.state.mission_failed'), isGlitchy: true },
                        ]);
                    }
    ...
    const handleTargetClick = useCallback((id: string) => {
        ...
        setScreenFlash('#8844ff'); setTimeout(() => setScreenFlash(null), 80);
        ...
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden border border-white/10 bg-black shadow-2xl">
            ...
            <div className="absolute inset-0 z-0 cursor-crosshair">
                <Canvas camera={{ position: [0, 0, 12], fov: 60 }} gl={{ antialias: false, powerPreference: "high-performance" }}>
                    ...
                    <fog attach="fog" args={['#040008', 8, 25]} />
                    <ambientLight intensity={0.3} />
                    <pointLight position={[10, 10, 10]} intensity={2} color="#8844ff" />
                    ...
                    <Sparkles count={300} scale={20} size={3} speed={0.5 + combo * 0.2} opacity={0.5} color="#8844ff" />
