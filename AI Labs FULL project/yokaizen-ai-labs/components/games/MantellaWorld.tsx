...
import { OrbitControls, Torus, MeshDistortMaterial, Float, Sparkles } from '@react-three/drei';
...
const CoreEntity = ({ isActive }: { isActive: boolean }) => {
    ...
    return (
        <Float speed={isActive ? 4 : 1.5} floatIntensity={1.5} rotationIntensity={isActive ? 3 : 1}>
            <Torus ref={meshRef} args={[1.5, 32, 100, 16]} scale={isActive ? 1.3 : 1}>
                <MeshDistortMaterial
                    color={new THREE.Color().setHSL(0.65, 0.9, isActive ? 0.7 : 0.4)}
                    envMapIntensity={1.5}
                    clearcoat={1}
        ...
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden border border-white/10 bg-black shadow-2xl">
            ...
            <div className="absolute inset-0 z-0 cursor-crosshair" onClick={handleInteract}>
                <Canvas camera={{ position: [0, 0, 10], fov: 60 }}>
                    ...
                    <pointLight position={[10, 10, 10]} intensity={1.5} color={new THREE.Color().setHSL(0.65, 1, 0.6)} />
                    <pointLight position={[-10, -10, -10]} intensity={0.8} color="#4f46e5" />
                    
                    <Sparkles count={300} scale={15} size={3} speed={0.5} opacity={0.6} color={new THREE.Color().setHSL(0.65, 1, 0.9)} />
