...
export interface MantellaWorldProps {
...
const TARGET_COLORS = ['#00ffcc', '#ffcc00', '#cc00ff', '#ffffff'];
const genPos = (): [number, number, number] => [(Math.random() - 0.5) * 12, (Math.random() - 0.5) * 8, (Math.random() - 0.5) * 6];

// --- Main Component ---
export const MantellaWorld: React.FC<MantellaWorldProps> = ({ onComplete, difficulty, t }) => {
    ...
    const [advisorMsg, setAdvisorMsg] = useState(t('game.advisor.core_holding'));
    ...
    useEffect(() => {
        queueDialogue([
            { id: `mantellaworld-brief-${Date.now()}`, character: 'ATHENA', text: t('game.instructions.click_targets') },
            { id: `mantellaworld-brief2-${Date.now()}`, character: 'SYNTAX', text: t('game.instructions.avoid_hazards'), isGlitchy: true },
        ]);
    }, [queueDialogue, t]);
    ...
    useEffect(() => {
        if (gameState !== 'PLAYING') return;
        const aLines = [t('game.advisor.core_holding'), t('game.advisor.metrics_align'), t('game.advisor.target_rate_optimal'), `${t('game.hud.combo_chain')}: ${combo}x`];
        const eLines = [t('game.adversary.core_exposed'), t('game.adversary.inevitability'), t('game.adversary.diverting_resources'), `${combo}x ${t('game.hud.combo')}...`];
                ...
                if (prev <= 1) {
                    ...
                    if (fs === 'SUCCESS') {
                        audio.playSuccess(); queueDialogue([
                            { id: `mantellaworld-win-${Date.now()}`, character: 'SYNTAX', text: `${t('game.hud.final_score')}: ${score}. ${t('game.hud.max_combo')}: ${maxCombo}x.` },
                            { id: `mantellaworld-win2-${Date.now()}`, character: 'ATHENA', text: t('game.state.core_secured') },
                        ]);
                    } else {
                        audio.playError(); queueDialogue([
                            { id: `mantellaworld-fail-${Date.now()}`, character: 'BYTE', text: t('game.state.mission_failed'), isGlitchy: true },
                        ]);
                    }
    ...
    const handleTargetClick = useCallback((id: string) => {
        ...
        setScreenFlash('#00ffcc'); setTimeout(() => setScreenFlash(null), 80);
        ...
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden border border-white/10 bg-black shadow-2xl">
                ...
                <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="absolute inset-0 z-20 flex items-center justify-center p-4 sm:p-0 bg-black/90 backdrop-blur-md">
                    <div className="text-center p-6 sm:p-10 w-full sm:w-auto rounded-2xl border border-white/10 bg-black/50 shadow-2xl max-w-[90vw] sm:max-w-md">
                        ...
                        <div className={`text-3xl sm:text-5xl font-black uppercase tracking-widest mb-4 ${gameState === 'SUCCESS' ? 'text-cyan-400' : 'text-red-500'} drop-shadow-[0_0_15px_currentColor]`}>
                            {gameState === 'SUCCESS' ? t('game.state.core_secured') : t('game.state.mission_failed')}
                        </div>
            ...
            <div className="absolute inset-0 z-0 cursor-crosshair">
                <Canvas camera={{ position: [0, 0, 12], fov: 60 }} gl={{ antialias: false, powerPreference: "high-performance" }}>
                    ...
                    <fog attach="fog" args={['#000804', 8, 25]} />
                    ...
                    <Sparkles count={300} scale={20} size={3} speed={0.5 + combo * 0.2} opacity={0.5} color="#00ffcc" />
                    <Sparkles count={100} scale={15} size={6} speed={1} opacity={0.3} color="#ffcc00" />
