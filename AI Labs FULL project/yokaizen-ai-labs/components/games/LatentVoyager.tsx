...
const HazardNode = ({ position, onClick, id }: {
    ...
    return (
        <mesh ref={meshRef} position={position}
            ...
            <pointLight distance={4} intensity={2.5} color="#ff0044" />
        </mesh>
    );
};
...
const TARGET_COLORS = ['#4488ff', '#ff4488', '#88ff44', '#ffffff'];
...
export const LatentVoyager: React.FC<LatentVoyagerProps> = ({ onComplete, difficulty, t }) => {
    ...
    useEffect(() => {
        queueDialogue([
            { id: `latentvoyager-brief-${Date.now()}`, character: 'BYTE', text: t('game.instructions.click_targets') },
            { id: `latentvoyager-brief2-${Date.now()}`, character: 'ATHENA', text: t('game.instructions.avoid_hazards'), isGlitchy: true },
        ]);
    }, [queueDialogue, t]);
    ...
    useEffect(() => {
        if (gameState !== 'PLAYING') return;
        const aLines = [t('game.advisor.wave_incoming'), t('game.advisor.flow_stable'), t('game.advisor.diversion_nodes'), `${t('game.hud.combo_chain')}: ${combo}x`];
        const eLines = [t('game.adversary.strategy_flawed'), t('game.adversary.wont_survive'), t('game.adversary.core_exposed'), `${combo}x ${t('game.hud.combo')}...`];
                ...
                if (prev <= 1) {
                    ...
                    if (fs === 'SUCCESS') {
                        audio.playSuccess(); queueDialogue([
                            { id: `latentvoyager-win-${Date.now()}`, character: 'SYNTAX', text: `${t('game.hud.final_score')}: ${score}. ${t('game.hud.max_combo')}: ${maxCombo}x.` },
                            { id: `latentvoyager-win2-${Date.now()}`, character: 'ATHENA', text: t('game.state.waves_analyzed') },
                        ]);
                    } else {
                        audio.playError(); queueDialogue([
                            { id: `latentvoyager-fail-${Date.now()}`, character: 'BYTE', text: t('game.state.core_breached'), isGlitchy: true },
                        ]);
                    }
    ...
    const handleTargetClick = useCallback((id: string) => {
        ...
        setScreenFlash('#4488ff'); setTimeout(() => setScreenFlash(null), 80);
        ...
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden border border-white/10 bg-black shadow-2xl">
            ...
            <div className="absolute inset-0 z-0 cursor-crosshair">
                <Canvas camera={{ position: [0, 0, 12], fov: 60 }} gl={{ antialias: false, powerPreference: "high-performance" }}>
                    ...
                    <Sparkles count={300} scale={20} size={3} speed={0.5 + combo * 0.2} opacity={0.5} color="#4488ff" />
                    <Sparkles count={100} scale={15} size={6} speed={1} opacity={0.3} color="#ff4488" />
