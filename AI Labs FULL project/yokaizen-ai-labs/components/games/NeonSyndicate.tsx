...
export interface NeonSyndicateProps {
    ...
    return (
        <Float speed={3} rotationIntensity={2} floatIntensity={2}>
            <mesh ref={meshRef} position={position}
                ...
                <meshStandardMaterial color={color} emissive={color} emissiveIntensity={hovered ? 5 : 3} toneMapped={false} />
...
const TARGET_COLORS = ['#ff0088', '#8800ff', '#ff44aa', '#ffffff'];
const genPos = (): [number, number, number] => [(Math.random() - 0.5) * 12, (Math.random() - 0.5) * 8, (Math.random() - 0.5) * 6];

// --- Main Component ---
export const NeonSyndicate: React.FC<NeonSyndicateProps> = ({ onComplete, difficulty, t }) => {
    ...
    const [advisorMsg, setAdvisorMsg] = useState(t('game.advisor.reflexes_exceed'));
    ...
    useEffect(() => {
        queueDialogue([
            { id: `neonsyndicate-brief-${Date.now()}`, character: 'BYTE', text: t('game.instructions.click_targets') },
            { id: `neonsyndicate-brief2-${Date.now()}`, character: 'SYNTAX', text: t('game.instructions.avoid_hazards'), isGlitchy: true },
        ]);
    }, [queueDialogue, t]);
    ...
    useEffect(() => {
        if (gameState !== 'PLAYING') return;
        const aLines = [t('game.advisor.reflexes_exceed'), t('game.advisor.flow_stable'), t('game.advisor.adversary_breach'), `${t('game.hud.combo_chain')}: ${combo}x`];
        const eLines = [t('game.adversary.dare_click'), t('game.adversary.tempting'), t('game.adversary.predictable'), `${combo}x ${t('game.hud.combo')}...`];
                ...
                if (prev <= 1) {
                    ...
                    if (fs === 'SUCCESS') {
                        audio.playSuccess(); queueDialogue([
                            { id: `neonsyndicate-win-${Date.now()}`, character: 'SYNTAX', text: `${t('game.hud.final_score')}: ${score}. ${t('game.hud.max_combo')}: ${maxCombo}x.` },
                            { id: `neonsyndicate-win2-${Date.now()}`, character: 'ATHENA', text: t('game.state.system_secured') },
                        ]);
                    } else {
                        audio.playError(); queueDialogue([
                            { id: `neonsyndicate-fail-${Date.now()}`, character: 'BYTE', text: t('game.state.breach_detected'), isGlitchy: true },
                        ]);
                    }
                    onComplete(score, { completionTime: 40 - prev, difficulty, maxCombo });
                    return 0;
                }
                return prev - 1;
            });
    ...
    const handleTargetClick = useCallback((id: string) => {
        ...
        setScreenFlash('#ff0088'); setTimeout(() => setScreenFlash(null), 80);
        ...
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden border border-white/10 bg-black shadow-2xl">
            ...
            <div className="absolute inset-0 z-0 cursor-crosshair">
                <Canvas camera={{ position: [0, 0, 12], fov: 60 }} gl={{ antialias: false, powerPreference: "high-performance" }}>
                    ...
                    <fog attach="fog" args={['#080004', 8, 25]} />
                    <ambientLight intensity={0.3} />
                    <pointLight position={[10, 10, 10]} intensity={2} color="#ff0088" />
                    ...
                    <Sparkles count={300} scale={20} size={3} speed={0.5 + combo * 0.2} opacity={0.5} color="#ff0088" />
