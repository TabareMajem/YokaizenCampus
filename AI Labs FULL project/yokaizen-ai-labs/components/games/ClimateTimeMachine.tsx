...
export interface ClimateTimeMachineProps {
...
const TargetNode = ({ position, color, scale, onClick, id }: {
    ...
    return (
        <Float speed={3} rotationIntensity={2} floatIntensity={2}>
            <mesh ref={meshRef} position={position} onClick={() => onClick(id)}>
...
const HazardNode = ({ position, onClick, id }: {
    ...
    return (
        <mesh ref={meshRef} position={position} onClick={() => onClick(id)}>
            ...
            <meshStandardMaterial color="#ff0000" emissive="#ff0000" emissiveIntensity={3} wireframe toneMapped={false} />
            <pointLight distance={4} intensity={2.5} color="#ff2200" />
        </mesh>
    );
};
...
const TARGET_COLORS = ['#00aaff', '#ff8800', '#00ccff', '#ffffff'];
const genPos = (): [number, number, number] => [(Math.random() - 0.5) * 12, (Math.random() - 0.5) * 8, (Math.random() - 0.5) * 6];

// --- Main Component ---
export const ClimateTimeMachine: React.FC<ClimateTimeMachineProps> = ({ onComplete, difficulty, t }) => {
    ...
    useEffect(() => {
        queueDialogue([
            { id: `climatetimemachine-brief-${Date.now()}`, character: 'ATHENA', text: t('game.instructions.click_targets') },
            { id: `climatetimemachine-brief2-${Date.now()}`, character: 'BYTE', text: t('game.instructions.avoid_hazards'), isGlitchy: true },
        ]);
    }, [queueDialogue, t]);
    ...
    useEffect(() => {
        if (gameState !== 'PLAYING') return;
        const aLines = [t('game.advisor.flow_stable'), t('game.advisor.metrics_align'), t('game.advisor.core_holding'), `${t('game.hud.combo_chain')}: ${combo}x`];
        const eLines = [t('game.adversary.inevitability'), t('game.adversary.diverting_resources'), t('game.adversary.hesitate'), `${combo}x ${t('game.hud.combo')}...`];
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            setTimeLeft(prev => {
                if (prev <= 1) {
                    clearInterval(timer);
                    const fs = score >= 350 ? 'SUCCESS' : 'FAILED';
                    setGameState(fs);
                    if (fs === 'SUCCESS') {
                        audio.playSuccess(); queueDialogue([
                            { id: `climatetimemachine-win-${Date.now()}`, character: 'SYNTAX', text: `${t('game.hud.final_score')}: ${score}. ${t('game.hud.max_combo')}: ${maxCombo}x.` },
                            { id: `climatetimemachine-win2-${Date.now()}`, character: 'ATHENA', text: t('game.state.core_secured') },
                        ]);
                    } else {
                        audio.playError(); queueDialogue([
                            { id: `climatetimemachine-fail-${Date.now()}`, character: 'BYTE', text: t('game.state.core_breached'), isGlitchy: true },
                        ]);
                    }
                    onComplete(score, { completionTime: 60 - prev, difficulty, maxCombo });
                    return 0;
                }
                return prev - 1;
            });
    ...
    const handleTargetClick = useCallback((id: string) => {
        ...
        setScreenFlash('#00aaff'); setTimeout(() => setScreenFlash(null), 80);
        ...
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden border border-white/10 bg-black shadow-2xl">
            ...
            <div className="absolute inset-0 z-0 cursor-crosshair">
                <Canvas camera={{ position: [0, 0, 12], fov: 60 }} gl={{ antialias: false, powerPreference: "high-performance" }}>
                    <color attach="background" args={['#1a1000']} />
                    <fog attach="fog" args={['#000408', 8, 25]} />
                    ...
                    <Sparkles count={300} scale={20} size={3} speed={0.5 + combo * 0.2} opacity={0.5} color="#00aaff" />
                    <Sparkles count={100} scale={15} size={6} speed={1} opacity={0.3} color="#ff8800" />
                    ...
                    <EffectComposer >
                        ...
                        />
                        <Glitch
                            delay={new THREE.Vector2(glitchActive ? 0 : 1.5, glitchActive ? 0 : 3.5)}
