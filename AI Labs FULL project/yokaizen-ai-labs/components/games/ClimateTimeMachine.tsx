    ...
    <div className="h-full flex flex-col bg-gray-950 relative overflow-hidden font-sans select-none">
        ...
        <div className="p-4 flex justify-between items-start z-20 relative bg-gradient-to-b from-black/90 to-transparent">
            ...
            <div className="text-right">
                ...
                <div className="text-xs text-gray-400 font-mono">{Math.round(co2)} PPM CO2</div>
            </div>
        </div>
        ...
        <div className="flex-1 relative flex items-center justify-center perspective-1000">
            {/* The Globe */}
            <div className={`relative w-64 h-64 rounded-full ${getPlanetColor()} transition-all duration-1000 shadow-inner overflow-hidden transform-style-3d`}>
                {/* Texture Overlay (Continents) */}
                <div className="absolute inset-0 opacity-60 mix-blend-multiply bg-[url('https://www.transparenttextures.com/patterns/stardust.png')]"></div>
                
                {/* Atmosphere / Clouds */}
                <div className="absolute inset-0 animate-spin-slow opacity-80">
                    <div className="absolute top-10 left-10 w-32 h-16 bg-white/20 blur-2xl rounded-full"></div>
                    <div className="absolute bottom-20 right-10 w-40 h-20 bg-white/20 blur-2xl rounded-full"></div>
                    {temp > 2.5 && <div className="absolute top-1/2 left-1/2 w-full h-full bg-red-500/10 blur-3xl"></div>}
                </div>

                {/* Smog Layer (Based on Industry) */}
                <div 
                    className="absolute inset-0 bg-gray-900 mix-blend-hard-light transition-opacity duration-500"
                    style={{ opacity: (industry / 200) + (getAtmosphereOpacity() * 0.5) }}
                ></div>

                {/* Rotating Shine */}
                <div className="absolute inset-0 bg-gradient-to-tr from-transparent to-white/10 rounded-full pointer-events-none"></div>
            </div>
                ...
                <div className="flex justify-between space-x-2">
                    {[1, 2, 3].map(p => (
                        <button
                            ...
                            className={`flex-1 py-3 rounded-xl text-[10px] font-bold uppercase border transition-all flex flex-col items-center justify-center ${
                                policies >= p 
                ...
                <div className="animate-in slide-in-from-bottom absolute inset-0 bg-black/95 flex flex-col items-center justify-center p-6 z-50 rounded-t-3xl">
                    <div className={`text-center p-6 rounded-2xl border w-full max-w-xs mb-6 ${temp < 2.0 ? 'bg-green-900/20 border-green-500' : 'bg-red-900/20 border-red-500'}`}>
                        ...
                        <p className="text-xs text-gray-300 font-mono">
                            {t('climate.final_temp')}: +{temp.toFixed(2)}°C<br/>
                            Year: {year}
                        </p>
                    </div>
