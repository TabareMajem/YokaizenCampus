...
const TARGET_COLORS = ['#00ff88', '#00ffff', '#44ffaa', '#ffffff'];
...
export const LatencyLab: React.FC<LatencyLabProps> = ({ onComplete, difficulty, t }) => {
    ...
    const [adversaryMsg, setAdversaryMsg] = useState(t('game.adversary.hesitate'));
    ...
    useEffect(() => {
        queueDialogue([
            { id: `latencylab-brief-${Date.now()}`, character: 'SYNTAX', text: t('game.instructions.click_targets') },
            { id: `latencylab-brief2-${Date.now()}`, character: 'ATHENA', text: t('game.instructions.avoid_hazards'), isGlitchy: true },
        ]);
    }, [queueDialogue, t]);
    ...
    useEffect(() => {
        if (gameState !== 'PLAYING') return;
        const aLines = [t('game.advisor.target_rate_optimal'), t('game.advisor.reflexes_exceed'), t('game.advisor.flow_stable'), `${t('game.hud.combo_chain')}: ${combo}x`];
        const eLines = [t('game.adversary.hesitate'), t('game.adversary.predictable'), t('game.adversary.dare_click'), `${combo}x ${t('game.hud.combo')}...`];
                ...
                if (prev <= 1) {
                    ...
                    if (fs === 'SUCCESS') {
                        audio.playSuccess(); queueDialogue([
                            { id: `latencylab-win-${Date.now()}`, character: 'SYNTAX', text: `${t('game.hud.final_score')}: ${score}. ${t('game.hud.max_combo')}: ${maxCombo}x.` },
                            { id: `latencylab-win2-${Date.now()}`, character: 'ATHENA', text: t('game.state.system_secured') },
                        ]);
                    } else {
                        audio.playError(); queueDialogue([
                            { id: `latencylab-fail-${Date.now()}`, character: 'BYTE', text: t('game.state.breach_detected'), isGlitchy: true },
                        ]);
                    }
    ...
    const handleTargetClick = useCallback((id: string) => {
        ...
        setScreenFlash('#00ff88'); setTimeout(() => setScreenFlash(null), 80);
        ...
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden border border-white/10 bg-black shadow-2xl">
            ...
            <div className="absolute inset-0 z-0 cursor-crosshair">
                <Canvas camera={{ position: [0, 0, 12], fov: 60 }} gl={{ antialias: false, powerPreference: "high-performance" }}>
                    ...
                    <Sparkles count={300} scale={20} size={3} speed={0.5 + combo * 0.2} opacity={0.5} color="#00ff88" />
                    <Sparkles count={100} scale={15} size={6} speed={1} opacity={0.3} color="#00ffff" />
