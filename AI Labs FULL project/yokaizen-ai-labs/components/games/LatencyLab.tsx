    ...
    <div className="h-full flex flex-col bg-[#0f172a] relative overflow-hidden font-mono select-none">
        ...
        <div className="flex-1 relative flex items-center justify-center p-6 perspective-[800px]">
            <div className="relative w-64 h-96 bg-gray-900 border-4 border-slate-700 rounded-xl shadow-2xl flex flex-col p-2 overflow-hidden transform rotate-y-12">
                ...
                {/* Server Blades */}
                {[1,2,3,4].map(i => (
                    <div key={i} className="flex-1 bg-black mb-2 rounded border border-slate-800 relative overflow-hidden group">
                        {/* Heat Map Overlay */}
                        <div 
                            className="absolute inset-0 bg-red-500 blur-xl transition-opacity duration-500"
                            style={{ opacity: Math.max(0, (temp - 50) / 50) * 0.8 }}
                        ></div>
                        ...
                        {isRunning && (
                            <div className="flex space-x-1 p-2">
                                {[1,2,3,4,5,6].map(j => (
                                    <div 
                                        key={j} 
                                        className="w-1 h-full bg-green-500 animate-pulse" 
                                        style={{ animationDuration: `${Math.random() * 0.5 + 0.1}s`, opacity: Math.random() }}
                                    ></div>
                                ))}
                            </div>
                        )}
                        ...
                        {/* Sparks if damaging */}
                        {temp > 90 && Math.random() > 0.8 && (
                            <div className="absolute top-1/2 left-1/2 w-2 h-2 bg-yellow-400 rounded-full blur-[1px] animate-ping"></div>
                        )}
                    </div>
                ))}

                {/* Fan Visual */}
                <div className="h-24 mt-auto relative flex justify-center items-center border-t border-slate-700 pt-2">
                    <Fan 
                        size={64} 
                        className={`text-slate-600 transition-all duration-1000 ${cooling > 2 ? 'text-blue-400' : ''}`} 
                        style={{ animation: isRunning ? `spin ${1/cooling}s linear infinite` : 'none' }}
                    />
                </div>
            </div>
        ...
        <div className="absolute top-4 right-4 w-48 space-y-4 bg-black/80 p-4 rounded-xl border border-slate-700 backdrop-blur z-10">
            <div>
                <div className="flex justify-between text-[10px] uppercase font-bold text-slate-400 mb-1">
                    ...
                    <span className={temp > 85 ? 'text-red-500 blink' : 'text-white'}>{Math.round(temp)}°C</span>
                </div>
            ...
            {!isRunning ? (
                ...
                <div className="text-center animate-in zoom-in">
                    ...
                    {alertMsg?.includes('TARGET') ? (
                        <p className="text-gray-400 text-xs">System stable. Efficiency maximized.</p>
                    ) : (
                        <Button variant="secondary" onClick={() => window.location.reload()}>{t('latency.reboot')}</Button>
                    )}
                </div>
            ) : (
