...
const HazardNode = ({ position, onClick, id }: {
    ...
    return (
        <mesh ref={meshRef} position={position}
            ...
            <meshStandardMaterial color="#ff0000" emissive="#ff0000" emissiveIntensity={3} wireframe toneMapped={false} transparent opacity={0.85} roughness={0.3} metalness={0.8} />
            <pointLight distance={4} intensity={2.5} color="#ff0055" />
        </mesh>
    );
};
...
const TARGET_COLORS = ['#00ffaa', '#ff00aa', '#00aaff', '#ffffff'];
...
export const BiasBingo: React.FC<BiasBingoProps> = ({ onComplete, difficulty, t }) => {
    ...
    useEffect(() => {
        queueDialogue([
            { id: `biasbingo-brief-${Date.now()}`, character: 'BYTE', text: t('game.instructions.click_targets') },
            { id: `biasbingo-brief2-${Date.now()}`, character: 'ATHENA', text: t('game.instructions.avoid_hazards'), isGlitchy: true },
        ]);
    }, [queueDialogue, t]);
    ...
    useEffect(() => {
        if (gameState !== 'PLAYING') return;
        const aLines = [t('game.advisor.flow_stable'), t('game.advisor.integrity_optimal'), t('game.advisor.reflexes_exceed'), `${t('game.hud.combo_chain')}: ${combo}x`];
        const eLines = [t('game.adversary.cracks_attention'), t('game.adversary.predictable'), t('game.adversary.tempting'), `${combo}x ${t('game.hud.combo')}...`];
                ...
                if (prev <= 1) {
                    ...
                    if (fs === 'SUCCESS') {
                        audio.playSuccess(); queueDialogue([
                            { id: `biasbingo-win-${Date.now()}`, character: 'SYNTAX', text: `${t('game.hud.final_score')}: ${score}. ${t('game.hud.max_combo')}: ${maxCombo}x.` },
                            { id: `biasbingo-win2-${Date.now()}`, character: 'ATHENA', text: t('game.state.system_secured') },
                        ]);
                    } else {
                        audio.playError(); queueDialogue([
                            { id: `biasbingo-fail-${Date.now()}`, character: 'BYTE', text: t('game.state.breach_detected'), isGlitchy: true },
                        ]);
                    }
    ...
    const handleTargetClick = useCallback((id: string) => {
        ...
        setScreenFlash('#00ffaa'); setTimeout(() => setScreenFlash(null), 80);
        ...
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden border border-white/10 bg-black shadow-2xl">
            ...
            <div className="absolute inset-0 z-0 cursor-crosshair">
                <Canvas camera={{ position: [0, 0, 12], fov: 60 }} gl={{ antialias: false, powerPreference: "high-performance" }}>
                    ...
                    <fog attach="fog" args={['#000808', 8, 25]} />
                    ...
                    <Sparkles count={300} scale={20} size={3} speed={0.5 + combo * 0.2} opacity={0.5} color="#00ffaa" />
                    <Sparkles count={100} scale={15} size={6} speed={1} opacity={0.3} color="#ff00aa" />
