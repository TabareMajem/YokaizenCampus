...
export interface NexusNegotiationProps {
...
const TARGET_COLORS = ['#ffaa44', '#44aaff', '#ff8800', '#ffffff'];
const genPos = (): [number, number, number] => [(Math.random() - 0.5) * 12, (Math.random() - 0.5) * 8, (Math.random() - 0.5) * 6];

// --- Main Component ---
export const NexusNegotiation: React.FC<NexusNegotiationProps> = ({ onComplete, difficulty, t }) => {
    ...
    useEffect(() => {
        queueDialogue([
            { id: `nexusnegotiation-brief-${Date.now()}`, character: 'ATHENA', text: t('game.instructions.click_targets') },
            { id: `nexusnegotiation-brief2-${Date.now()}`, character: 'BYTE', text: t('game.instructions.avoid_hazards'), isGlitchy: true },
        ]);
    }, [queueDialogue, t]);
    ...
    useEffect(() => {
        if (gameState !== 'PLAYING') return;
        const aLines = [t('game.advisor.flow_stable'), t('game.advisor.integrity_optimal'), t('game.advisor.diversion_nodes'), `${t('game.hud.combo_chain')}: ${combo}x`];
        const eLines = [t('game.adversary.strategy_flawed'), t('game.adversary.cracks_attention'), t('game.adversary.hesitate'), `${combo}x ${t('game.hud.combo')}...`];
                ...
                if (prev <= 1) {
                    ...
                    if (fs === 'SUCCESS') {
                        audio.playSuccess(); queueDialogue([
                            { id: `nexusnegotiation-win-${Date.now()}`, character: 'SYNTAX', text: `${t('game.hud.final_score')}: ${score}. ${t('game.hud.max_combo')}: ${maxCombo}x.` },
                            { id: `nexusnegotiation-win2-${Date.now()}`, character: 'ATHENA', text: t('game.state.mission_complete') },
                        ]);
                    } else {
                        audio.playError(); queueDialogue([
                            { id: `nexusnegotiation-fail-${Date.now()}`, character: 'BYTE', text: t('game.state.mission_failed'), isGlitchy: true },
                        ]);
                    }
    ...
    const handleTargetClick = useCallback((id: string) => {
        ...
        setScreenFlash('#ffaa44'); setTimeout(() => setScreenFlash(null), 80);
        ...
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden border border-white/10 bg-black shadow-2xl">
            ...
            <div className="absolute inset-0 z-0 cursor-crosshair">
                <Canvas camera={{ position: [0, 0, 12], fov: 60 }} gl={{ antialias: false, powerPreference: "high-performance" }}>
                    <color attach="background" args={['#060400']} />
                    <fog attach="fog" args={['#060400', 8, 25]} />
                    ...
                    <Sparkles count={300} scale={20} size={3} speed={0.5 + combo * 0.2} opacity={0.5} color="#ffaa44" />
                    <Sparkles count={100} scale={15} size={6} speed={1} opacity={0.3} color="#44aaff" />
