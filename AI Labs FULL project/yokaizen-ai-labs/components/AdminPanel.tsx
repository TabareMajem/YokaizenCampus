...
import { Trash2, Gift, BarChart3, Gamepad2, Users, Plus, Sparkles, Activity, AlertTriangle, Check, XCircle, Smartphone, Monitor, Bug, Zap, Search, ShieldAlert, Loader2 } from 'lucide-react';
import { getAdminStats, getAdminRewards, getDailyTraffic, createReward, deleteReward, DailyData } from '../services/adminService';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ games, onUpdateGames, users, onUpdateUsers }) => {
    ...
    const [stats, setStats] = useState<AdminStats>({ dau: 0, mau: 0, totalRevenue: 0, activeGames: games.length });
    const [trafficData, setTrafficData] = useState<DailyData[]>([]);
    ...
    useEffect(() => {
        const fetchAdminData = async () => {
            setLoading(true);
            try {
                const [statsData, rewardsData, traffic] = await Promise.all([
                    getAdminStats(),
                    getAdminRewards(),
                    getDailyTraffic()
                ]);
                setStats({ ...statsData, activeGames: games.length });
                setRewards(rewardsData);
                setTrafficData(traffic.length > 0 ? traffic : [
                            ...
                            <>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-4 rounded-xl bg-gray-900 border border-gray-800">
                                        ...
                                        <div className="text-2xl font-mono font-bold text-green-400">{stats.dau.toLocaleString()}</div>
                                    </div>
                                    <div className="p-4 rounded-xl bg-gray-900 border border-gray-800">
                                        <div className="text-xs text-gray-500 uppercase">Revenue</div>
                                        <div className="text-2xl font-mono font-bold text-amber-400">${stats.totalRevenue.toLocaleString()}</div>
                                    </div>
                                </div>
