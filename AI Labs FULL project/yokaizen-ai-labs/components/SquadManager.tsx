...
export const SquadManager: React.FC<SquadManagerProps> = ({ squads, userSquadId, onJoinSquad, onCreateSquad, isPro, onTriggerPaywall, t, user, onUpdateUser, onUpdateSquads }) => {
  ...
  const handleInvite = () => {
      ...
      if (navigator.share) {
          navigator.share({
              title: 'Join my Yokaizen Squad',
              text: `Join ${userSquad?.name} and dominate the leaderboard!`,
              url: inviteLink,
          }).catch(() => {
  ...
  const handleLeaveSquad = () => {
      if (confirm("Are you sure you want to leave this unit?")) {
  ...
  const handleAttackBoss = () => {
      ...
      setTimeout(() => {
          ...
          // Calculate Damage
          const baseDmg = 1000 + Math.floor(Math.random() * 500); // ~1000 dmg
          ...
          // Persist to "Backend"
          if (userSquadId) {
              localStorage.setItem(`raid_boss_${userSquadId}`, JSON.stringify({ hp: newHp, maxHp: maxBossHp }));
              localStorage.setItem(`last_attack_${user.id}`, Date.now().toString());
          }
          
          setMissionResult({ xp: 500, credits: 100, damage });
          ...
          if (onUpdateSquads && userSquad) {
              const updatedSquad = { 
                  ...userSquad, 
                  totalXp: userSquad.totalXp + 500,
                  weeklyQuestProgress: Math.min(userSquad.weeklyQuestTarget, userSquad.weeklyQuestProgress + damage / 100) 
              };
              const newSquads = squads.map(s => s.id === userSquad.id ? updatedSquad : s);
              onUpdateSquads(newSquads);
          }
          
          if (crit > 1) audio.playSuccess(); else audio.playClick();
          setView('REPORT');
      }, 1500);
  };
                            ...
                            <div>
                                ...
                                <div className="flex space-x-2 mt-1.5">
                                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border uppercase ${
                                        squad.tier === 'Elite' ? 'border-amber-500 text-amber-500 bg-amber-500/10' : 
  ...
  if (view === 'WAR_ROOM') {
      return (
          <div className="p-4 h-full flex flex-col bg-black/90 animate-in zoom-in relative overflow-hidden">
              ...
              <div className="text-center mb-6 border-b border-red-900/30 pb-4 relative z-10 flex justify-between items-center">
                  ...
                  <div>
                      ...
                      <p className="text-[10px] text-red-400/70 font-mono mt-1">RAID TARGET: OMEGA-CORE</p>
                  </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center relative z-10">
                  ...
                  <div className={`relative w-64 h-64 flex items-center justify-center ${isDeploying ? 'animate-shake' : ''}`}>
                  ...
                  {bossHp <= 0 && (
                      <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-20 animate-in zoom-in">
                          <h2 className="text-4xl text-green-500 font-black glitch-text">TARGET DESTROYED</h2>
                      </div>
                  )}
              </div>
              ...
              <div className="mt-4 space-y-3 relative z-10">
                  <Button 
                    ...
                    className="h-16 text-xl font-black tracking-widest shadow-[0_0_30px_rgba(220,38,38,0.4)] border-2 border-red-500 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                      {cooldown > 0 ? `RECHARGING (${formatCooldown(cooldown)})` : isDeploying ? 'ATTACKING...' : 'LAUNCH RAID ATTACK'}
                  </Button>
                  <p className="text-center text-[10px] text-gray-500 font-mono">1,000 Damage / Attack. Requires Squad Coordination.</p>
              </div>
          </div>
      );
  }
          ...
          <div className="p-6 h-full flex flex-col items-center justify-center bg-black animate-in zoom-in text-center">
              ...
              <div className="w-full max-w-xs bg-gray-900 border border-white/10 rounded-xl p-6 mt-8 space-y-4">
                  <div className="flex justify-between items-center border-b border-gray-800 pb-2">
                      <span className="text-xs text-gray-400 font-bold uppercase">DAMAGE DEALT</span>
