...
export const AgentBuilder: React.FC<AgentBuilderProps> = ({ agent: initialAgent, onSave, onDelete, checkCooldown, t = (k) => k }) => {
  ...
  const handleShare = () => {
      ...
      alert("Agent Neural Key copied to clipboard! Share this URL to let others import your agent.");
      audio.playSuccess();
  };

  const handleGenerateAvatar = async () => {
      if (!formData.persona && !formData.name) {
          alert("Please enter a Name or Persona first to generate an avatar.");
          return;
      }
      ...
      try {
          ...
          alert("Avatar generation failed. Try again.");
          audio.playError();
      } finally {
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {availableModels.map(m => (
                            <button
                                ...
                                className={`relative p-3 rounded-xl border-2 text-left transition-all group overflow-hidden ${
                                    formData.model === m.id 
                        ...
                        return (
                            <div key={i} className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2`}>
                                <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} max-w-[85%]`}>
                                    <div className={`p-3.5 rounded-2xl text-sm shadow-md leading-relaxed ${
                                        isUser 
