...
export const GameCover: React.FC<GameCoverProps> = ({ game, className = '', iconSize = 120 }) => {
  ...
  const getEngineCover = () => {
    switch (game.type) {
      // Engine A: Node Network
      case GameType.PHANTOM_LATENCY:
      case GameType.AETHELRED_GAMBIT:
      case GameType.NEON_SYNDICATE:
      case GameType.COGNITIVE_CITY:
      case GameType.XENOFLORA:
      case GameType.CAUSAL_CONSERVATORY:
      case GameType.RED_TEAM:
      case GameType.PLANARIUM_HEIST:
        return '/assets/aaa/covers/engines/engine_a.png';

      // Engine B: Terminal Sandbox
      case GameType.PROMPT_ARCHITECT:
      case GameType.TOKEN_TSUNAMI:
      case GameType.PERSONA_SWITCHBOARD:
      case GameType.DATA_WHISPERER:
      case GameType.ENTROPY_SANDBOX:
      case GameType.VAMPIRE_INVITATION:
      case GameType.STYLE_ANCHOR:
      case GameType.BANDIT_BISTRO:
        return '/assets/aaa/covers/engines/engine_b.png';

      // Engine C: Endless Runner/Tunnel
      case GameType.NEON_DRIFT:
      case GameType.LATENCY_LAB:
      case GameType.PROMPT_DRIFT:
      case GameType.GRADIENT_SKI:
      case GameType.LATENT_VOYAGER:
      case GameType.GLITCHWAVE:
      case GameType.CHRONO_QUEST:
      case GameType.DREAM_SIM:
      case GameType.SPACE_MISSION:
      case GameType.CYBER_RAIN:
        return '/assets/aaa/covers/engines/engine_c.png';

      // Engine D: Deduction/Investigation
      case GameType.DEEPFAKE_DETECTIVE:
      case GameType.NEURAL_NOIR:
      case GameType.DOPPELGANGER:
      case GameType.VOIGHT_KAMPFF:
      case GameType.OOD_SENTINEL:
      case GameType.BIAS_BINGO:
      case GameType.REWARD_FIXER:
      case GameType.MANTELLA:
      case GameType.BIO_GUARD:
      case GameType.PROTEIN_POKER:
        return '/assets/aaa/covers/engines/engine_d.png';

      // Engine E: Resource Defense
      case GameType.DEFENSE_STRATEGIST:
      case GameType.SMART_CITY_MAYOR:
      case GameType.ARRAKIS_SANDS:
      case GameType.CLIMATE_TIME_MACHINE:
      case GameType.WALL_STREET_WAR:
      case GameType.LAZARUS_VECTOR:
      case GameType.NEXUS_NEGOTIATION:
        return '/assets/aaa/covers/engines/engine_e.png';

      default:
        // Fallback simple assignment based on String hash
        const engines = ['a', 'b', 'c', 'd', 'e'];
        const hash = game.id.split('').reduce((a, b) => { a = ((a << 5) - a) + b.charCodeAt(0); return a & a }, 0);
        const engineId = engines[Math.abs(hash) % 5];
        return `/assets/aaa/covers/engines/engine_${engineId}.png`;
    }
  };
  ...
  return (
    <div className={`relative overflow-hidden bg-gray-900 ${className}`}>
      ...
      {/* 2. Color Tint Layer (Subtle Gradient instead of Multiply) */}
      <div
        className="absolute inset-0 z-0 opacity-40 mix-blend-color"
        style={getGradientStyle()}
      ></div>

      {/* Modern dark gradient from bottom for text readability */}
      <div className="absolute inset-0 z-0 bg-gradient-to-t from-black via-black/40 to-transparent"></div>

      {/* Fallback Glowing Icon if Image fails */}
      {(imgError || !imageUrl) && (
        <div className="absolute inset-0 flex items-center justify-center opacity-40 group-hover:opacity-60 transition-opacity z-10">
          <div className="drop-shadow-[0_0_20px_white]">
            {getIcon()}
          </div>
        </div>
      )}
      ...
      <div className="absolute inset-0 z-0 bg-gradient-to-tr from-transparent via-white/5 to-white/10 opacity-30 mix-blend-overlay"></div>
      ...
      {/* 5. Large Watermark Icon - Only show clearly if no image */}
      <div className={`absolute -right-8 -bottom-8 transform rotate-[-15deg] text-white pointer-events-none z-10 mix-blend-overlay transition-opacity duration-300 ${imgError || !imageUrl ? 'opacity-40' : 'opacity-10'}`}>
        {getIcon()}
      </div>
