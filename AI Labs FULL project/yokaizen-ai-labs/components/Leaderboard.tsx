...
import { Trophy, Globe, Users, Shield, Medal, Loader2 } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { TRANSLATIONS } from '../translations';
import { getGlobalLeaderboard, getSquadLeaderboard, getRegionalLeaderboard, LeaderboardEntry } from '../services/leaderboardService';
            ...
            try {
                ...
                switch (filter) {
                    case 'GLOBAL':
                        ...
                        break;
                    case 'SQUAD':
                        data = await getSquadLeaderboard();
                        break;
                    case 'REGION':
                        data = await getRegionalLeaderboard(user?.language === 'JP' ? 'JP' : 'US');
                        break;
                }
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                {loading ? (
                    ...
                    <div className="flex items-center justify-center h-40 text-gray-500">
                        <p className="text-sm">{t('leaderboard.empty') || 'No rankings yet'}</p>
                    </div>
                ) : (
                    leaderboardData.map((entry) => (
                        <div
                            key={entry.rank}
                            className={`flex items-center p-3 rounded-xl border transition-all hover:scale-[1.01] ${entry.isUser
