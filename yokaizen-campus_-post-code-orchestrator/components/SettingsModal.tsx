...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, language = Language.EN }) => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        if (userProfile.apiKeys?.google) {
          setApiKey(userProfile.apiKeys.google);
        }
      } catch (e) {
    ...
    // Also check school mode which is still LocalStorage driven for now
  ...
  const handleSave = async () => {
    ...
    try {
      if (isSchoolMode) {
        ...
        await API.user.updateApiKeys({ google: "" });
      } else {
        if (apiKey.trim()) {
          await API.user.updateApiKeys({ google: apiKey.trim() });
        }
