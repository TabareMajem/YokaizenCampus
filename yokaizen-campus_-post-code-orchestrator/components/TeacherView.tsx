...
import { 
  ...
  Terminal, Globe, LogOut, Settings, Sparkles, FileJson, Info, X, Play, Save
} from 'lucide-react';
...
export const TeacherView: React.FC<TeacherViewProps> = ({ currentMode, onModeChange, onSwitchToStudent, language, onLanguageChange, user, onLogout }) => {
  ...
  const handleBroadcast = async () => {
      ...
      setBroadcastMsg("");
      alert(TC.BROADCAST_SENT);
  };

  const handleDeploy = () => {
    if(!generatedLesson) return;
    alert(TC.DEPLOYED_MSG);
    // In real app, push to API
  };

  const handleSaveDraft = () => {
    if(!generatedLesson) return;
    alert(TC.DRAFT_SAVED);
  ...
  return (
    <div className={`w-full h-full font-mono flex flex-col overflow-hidden transition-colors duration-500 ${currentMode === PhilosophyMode.FINLAND ? 'bg-[#1c1917] text-stone-300 font-serif' : 'bg-slate-950 text-slate-200'}`}>
      ...
      {/* Top Bar */}
      <div className={`flex justify-between items-center px-6 py-4 border-b ${currentMode === PhilosophyMode.KOREA ? 'border-red-900 bg-red-950/10' : 'border-slate-800 bg-slate-900/50'}`}>
        <div className="flex items-center gap-4">
          <div className={`p-2 rounded ${getThemeColor()}`}>
            <Activity className="w-6 h-6" />
          </div>
          <div>
            <h1 className="text-xl font-bold tracking-widest">{T.BRIDGE_COMMAND}</h1>
            <p className="text-[10px] opacity-60 uppercase">{user.name} // {user.role}</p>
          </div>
        </div>
        ...
        <div className="flex gap-4 items-center">
           ...
           <button onClick={() => setIsSettingsOpen(true)} className="p-2 border border-slate-700 hover:bg-slate-800 rounded transition-colors" title="Settings">
              <Settings className="w-4 h-4" />
           </button>

           <button onClick={onSwitchToStudent} className="px-4 py-2 border border-slate-700 hover:bg-slate-800 text-xs rounded transition-colors flex items-center gap-2">
            <Eye className="w-3 h-3" /> {T.STUDENT_VIEW}
          </button>
      ...
      <div className="flex-1 grid grid-cols-12 gap-6 p-6 overflow-hidden">
        ...
        <div className="col-span-6 flex flex-col overflow-hidden">
          ...
          {activeTab === 'LIVEOPS' && (
            <div className={`h-full rounded-xl border p-6 flex flex-col ${currentMode === PhilosophyMode.KOREA ? 'bg-black border-red-900' : 'bg-slate-900 border-slate-800'}`}>
            ...
            <div className={`h-full rounded-xl border p-6 flex flex-col ${currentMode === PhilosophyMode.KOREA ? 'bg-black border-red-900' : 'bg-slate-900 border-slate-800'}`}>
              ...
              <div className="flex gap-2 mb-6">
                ...
                <button 
                  ...
                  className="bg-purple-600 hover:bg-purple-500 text-white px-6 rounded font-bold transition-colors disabled:opacity-50 flex items-center gap-2"
                >
        ...
        <div className="col-span-3 flex flex-col gap-6">
           ...
           <div className={`flex-1 p-4 rounded-xl border relative overflow-hidden ${currentMode === PhilosophyMode.KOREA ? 'bg-black border-red-900' : 'bg-slate-900 border-slate-800'}`}>
              {selectedStudent ? (
