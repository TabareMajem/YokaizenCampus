...
export const AdminView: React.FC<AdminViewProps> = ({ user, onLogout, language, setLanguage }) => {
  ...
  const handleLicenseSave = () => {
    ...
    showNotification(TC.SAVED_MSG, 'success');
    loadData();
  };
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans flex flex-col">
      ...
      {notification && (
        <div className={`fixed top-20 right-8 z-50 px-6 py-3 rounded shadow-xl border animate-in slide-in-from-right ${notification.type === 'success' ? 'bg-green-900 border-green-500 text-green-100' : 'bg-red-900 border-red-500 text-red-100'}`}>
